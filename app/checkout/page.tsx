    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans" style={{ backgroundColor: '#0a0a0a' }}>
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-neutral-900 border border-neutral-700 p-8 rounded-2xl max-w-sm w-full text-center shadow-2xl">
      ...
      {statusSewa === 'success' ? (
        ...
        <div className="min-h-screen bg-neutral-950 text-white font-sans flex items-center justify-center p-6" style={{ backgroundColor: '#0a0a0a' }}>
          <div className="bg-neutral-900 p-8 rounded-2xl border border-orange-500 max-w-md w-full text-center shadow-xl" style={{ backgroundColor: '#171717', borderColor: '#ea580c', borderRadius: '16px' }}>
            ...
            <span className="px-3 py-1 bg-orange-950 text-orange-400 text-xs font-semibold rounded-full border border-orange-800 uppercase tracking-wider">Menunggu Pembayaran</span>
            <h2 className="text-xl font-bold mt-4 mb-2 text-white">Selesaikan Pembayaran Kamu</h2>
            <p className="text-sm text-gray-400 mb-6" style={{ color: '#a3a3a3' }}>Alat outdoor kamu sudah **Terbooking otomatis**. Silakan selesaikan transaksi agar status berubah.</p>
        ...
        <>
          <nav className="border-b border-gray-800 bg-neutral-900 px-6 py-4 flex justify-between items-center" style={{ backgroundColor: '#141414', borderColor: '#262626' }}><Link href="/" className="text-xl font-bold text-green-500" style={{ color: '#22c55e' }}>fynooOutdoorRent</Link><Link href="/katalog" className="text-sm text-gray-400 hover:text-white"> Kembali ke Katalog</Link></nav>
          <div className="max-w-4xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
            <div className="bg-neutral-900 p-6 rounded-xl border border-gray-800" style={{ backgroundColor: '#171717', borderColor: '#262626', borderRadius: '12px' }}>
              ...
              <form onSubmit={buatBookingPending} className="space-y-4">
                <div><label className="text-sm text-gray-400 block mb-1">Nama Lengkap Sesuai KTP:</label><input type="text" required value={formData.namaPenyewa} onChange={(e) => setFormData({...formData, namaPenyewa: e.target.value})} className="w-full bg-neutral-800 border border-gray-700 rounded p-2 text-white text-sm" style={{ backgroundColor: '#262626', borderColor: '#404040' }}/></div>
                <div><label className="text-sm text-gray-400 block mb-1">Nomor WhatsApp:</label><input type="tel" required value={formData.nomorHp} onChange={(e) => setFormData({...formData, nomorHp: e.target.value})} className="w-full bg-neutral-800 border border-gray-700 rounded p-2 text-white text-sm" style={{ backgroundColor: '#262626', borderColor: '#404040' }}/></div>
            ...
            <div className="bg-neutral-900 p-6 rounded-xl border border-gray-800 h-fit" style={{ backgroundColor: '#171717', borderColor: '#262626', borderRadius: '12px' }}>
              ...
              <div className="divide-y divide-gray-800 mb-4" style={{ borderColor: '#262626' }}>{keranjang.map((item) => (<div key={item._id} className="py-3 flex justify-between items-center text-sm"><div><h4 className="font-medium text-gray-200 capitalize">{item.name}</h4><p className="text-xs text-gray-400">{item.qty} unit x Rp {Number(item.pricePerDay || 0).toLocaleString('id-ID')}/hari</p></div><span className="font-semibold text-gray-100">Rp {(item.pricePerDay * item.qty).toLocaleString('id-ID')}</span></div>))}</div>
              <div className="border-t border-gray-800 pt-4 space-y-2 text-sm" style={{ borderColor: '#262626' }}><div className="flex justify-between text-gray-400"><span>Durasi Sewa:</span><span>{durasiSewa} Hari</span></div><div className="flex justify-between font-bold text-lg border-t border-gray-800 pt-2 text-white" style={{ borderColor: '#262626' }}><span>Total Pembayaran:</span><span className="text-orange-500" style={{ color: '#f97316' }}>Rp {hitungTotalSemua().toLocaleString('id-ID')}</span></div></div>
            </div>
          </div>
        </>
      )}
